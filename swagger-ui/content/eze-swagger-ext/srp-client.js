...
SRPClient = function (username, password, group, hashFn) {
  ...
  this.hexPrefix = '0x';
  this.privA = this.srpRandom();
  this.SRP_A = this.calculateA();
  this.clientKey = "";
  this.Mc = "";
  this.iv_len = 32;
  this.key_len = 64;
...
SRPClient.prototype = {
  ...
  calculateMc: function(srp_s, srp_b) {
    if (!srp_s || !srp_b)
      throw 'Missing parameters.';
	var srp_B = new BigInteger(srp_b, 10),
	    srp_U = this.calculateU(this.SRP_A, srp_B),
	    clientSecret = this.calculateS(srp_B, srp_s, srp_U, this.privA);
		
	this.clientKey = this.calculateK(clientSecret);
	this.Mc = this.concastMc(srp_s, this.SRP_A, srp_B, this.clientKey);
	return this.Mc;
  },
  
  calculateMs: function() {
	if (!this.Mc || !this.clientKey || !this.SRP_A)
		throw 'Missing parameters.';
	return this.concastMs(this.SRP_A, this.Mc, this.clientKey);
  },
  ...
  srpRandom: function() {
    ...
    if (r.bitLength() < 255) {
		//It barely failed, just in case, generate again;
		var numTry = 0;
		while (+numTry < 2 && r.bitLength() < 255) {
			words = sjcl.random.randomWords(8,0);
			hex = sjcl.codec.hex.fromBits(words);
  ...
  N_XOR_g: function () {
    var binN = this.hexHash(this.N.toHex()),
	    binG = this.hexHash(this.g.toHex()),
		rtnVal = [],
		len = binN.length,
		i = 0;
	for (; i < len; i++) {
  ...
  */
  ...
  /*
   * Return the ASCII code for each char
   */
  ascii: function(str) {
	var rtnASCII = [],
	    i = 0,
	...
	while (i < len)
	{
		rtnASCII.push(str.charCodeAt(i++));
	}
	return rtnASCII;
  },
  ...
  fillStr: function(str, len) {
	var rtn = [];
	rtn.push(str);
	for (var i = 0; i <= Math.floor(len / str.length); i++)
	{
	   rtn.push(str);
	}
	return rtn.join('').substr(0, len);
  },
  ...
  /* Encrypt the plain text by AES, the IV is from EphA, Key is the ClientKey, /*
  /*   which is indistinguishable from each successful login.  */
  encryptPlainText_Aes: function(text, strClientKey, strSRP_A)
  {
	  if ( !text || typeof(text) !== 'string' )
		throw 'plaintext has to be a non-empty string';
	  strClientKey = strClientKey || this.clientKey;
	  strSRP_A = strSRP_A || this.SRP_A.toString();
	  if ( !strClientKey || typeof(strClientKey) !== 'string' )
		throw 'ClientKey has to be a non-empty string';
	  if ( !strSRP_A || typeof(strSRP_A) !== 'string' )
		throw 'SRP_A has to be a non-empty (BigInteger) string';
	  var strIV = this.fillStr(strSRP_A, this.iv_len),
		  iv = CryptoJS.enc.Utf8.parse(strIV.toLowerCase()),
		  strKey = this.fillStr(strClientKey, this.key_len),
		  key = CryptoJS.enc.Hex.parse(strKey.toLowerCase()),
		  encrypted = {};
	  try {
